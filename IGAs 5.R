...
# SUB_BD_INAF=cbind(SUB_BD_INAF,"UNIV_UF"=c(960,1153,5256,1715,1019,474,3680),"UNIV_UFC"=c(132,21, 21, 342, 36, 153, 84 ))
...
#   scale_fill_manual(values=c(OEFA_1,OEFA_3,OEFA_6,OEFA_5, OEFA_7, OEFA_4,OEFA_2))+
...
# SUB_BD_HISTORIAL$AUX1=1
# SUB_BD_HISTORIAL$AUX2=0
# #MARCAR AQUELLOS CASOS OBSERVADOS y PEGARLOS EN EL AUXILIAR
# SUB_BD_HISTORIAL$AUX2[SUB_BD_HISTORIAL$PROC_ESTADO=="OBSERVADO"]=1
# #GENERANDO EL MES DE REVISION
# SUB_BD_HISTORIAL$MES=month(SUB_BD_HISTORIAL$F_ESTADO)
# #GENERAR EL SUMARIZE DE CONTEO DE CASOS GENERALES Y CASOS CON ALGUNA OBSERVACION (TABLA POR SUBSECT, ESTADO Y COORD)
...
# #PARA TODOS LOS CASOS QUE TENGAN AL MENOS UNA OBSERVACION, SE COLOCA 1, LOS DEMÁS 0
# SUB_BD_HISTORIAL$AUX2[SUB_BD_HISTORIAL$AUX2>=1]=1
# #SE GENERA EL AUXILIAR DE CASOS EN CSIG
# SUB_BD_HISTORIAL$AUX3=0
# SUB_BD_HISTORIAL$AUX3[SUB_BD_HISTORIAL$ETAPA=="3) ESPECIALISTA CSIG"]=1
# #SE GENERA AUXILIAR QUE INDICA LOS CASOS OBSERVADOS EN COORDINACION
# SUB_BD_HISTORIAL$AUX4=0
# SUB_BD_HISTORIAL$AUX4[SUB_BD_HISTORIAL$ETAPA=="2) COORDINADOR"&SUB_BD_HISTORIAL$AUX2==1]=1
# #SE GENERA AUXILIAR  QUE INDICA LOS CASOS OBSERVADOS EN CSIG
# SUB_BD_HISTORIAL$AUX5=0
# SUB_BD_HISTORIAL$AUX5[SUB_BD_HISTORIAL$ETAPA=="3) ESPECIALISTA CSIG"&SUB_BD_HISTORIAL$AUX2==1]=1
# #CREANDO EL SUMARIZE DE CONTEO DE CASOS CON OBSERVACIONES POR IGA
...
# #VOLVIENDO A INDICAR LOS CASOS CON AL MENOS UNA OBSERVACIÓN
# SUB_BD_HISTORIAL$AUX2[SUB_BD_HISTORIAL$AUX2>=1]=1
# #GENERANDO AUXILIAR DE CASOS EN COORDINACIÓN (TODOS ESTOS CASOS HAN PASADO POR COORDINACIÓN)
# SUB_BD_HISTORIAL$CASOS_COORD=1
# #GENERANDO LOS SUMARIZE
...
BUBBLE_GRAPH=ggplot(BD_RESUMEN)+
  annotate("rect", xmin = 0, xmax = 60, ymin = 0, ymax = 40, 
           alpha = 0.1, fill = "firebrick") + 
  annotate("rect", xmin = 0, xmax = 100, ymin = 10, ymax = 40, 
           alpha = 0.1, fill = "firebrick") + 
  geom_point(aes(x=AVANCE_IGA,y=PORC_CASOS_OBS_CSIG,size=TOT_IGA_INAF,color=SUB_SECT), alpha=0.8)+
  scale_color_manual(values=c(OEFA_1,OEFA_3,OEFA_6,OEFA_5,OEFA_7,OEFA_4, OEFA_2))+
  ggeasy::easy_all_text_size(size = 15)+
  theme_minimal()+
  ...
  scale_y_continuous(limits = c(0, 40), breaks = seq(0,100,10), expand = c(0, 0))+
  ...
  geom_text( x = 50, y = 40, angle = 90, label = paste("Umbral en llenado (60%)", sep=""), 
  ...
  ggeasy::easy_text_size(c("legend.text"),size = 15)
...
BUBBLE_GRAPH=ggplot(BD_RESUMEN)+
  ...
  geom_text( x = 60, y = 40, angle = 90, label = paste("Umbral en llenado (60%)", sep=""), 
             hjust = 1,vjust = -0.5,size =5)+
  geom_text( x = 100, y = 10, angle = 0, label = paste("Umbral en observaciones (10%)", sep=""), 
             hjust = 1,vjust = -0.5,size =5)+
  guides(color = guide_legend(override.aes = list(size = 5)))+
...
write.csv(SUB_BD_INAF1,"C:\\Users\\lfpal\\Desktop\\File.csv", row.names = FALSE)
...
AVANCE_REG_PERCAP_DIA <- ggplot(SUB_BD_INAF3,aes(F_REG, (REGISTROS_PERCAP), group = SUB_SECT, color = factor(SUB_SECT))) +
  ...
  scale_color_manual(values=c(OEFA_1,OEFA_3,OEFA_2,OEFA_5, OEFA_7, OEFA_4, OEFA_2))+
  ylim(0,40)+
  labs(x="",
...
AVANCE_REG_PERCAP_DIA <- ggplot(SUB_BD_INAF5,aes(F_REG, (N_REG), group = SUB_SECT, color = factor(SUB_SECT))) +
  geom_step(size=1) +
  geom_smooth(method = "loess", formula = y~x, se=T, color= "firebrick",fill="gray70")+
  scale_color_manual(values=c(OEFA_1,OEFA_3,OEFA_2,OEFA_5, OEFA_7, OEFA_4, OEFA_6))+
  #theme_minimal()+
  #ylim(0,15)+
...
if (all(paste(SUB_BD_INAF9$Etapa,SUB_BD_INAF9$Estado)!="2) COORDINADOR EN REVISION")==T) {SUB_BD_INAF9=rbind(SUB_BD_INAF9,c("Coordinador","En revisión",0,"0%"))}
...
#CORRIGIENDO "ERRORES" DE LA DATA
if (all(paste(SUB_BD_INAF10$Etapa,SUB_BD_INAF10$Estado)!="2) COORDINADOR EN REVISION")==T) {SUB_BD_INAF10=rbind(SUB_BD_INAF10,c("Coordinador","En revisión",0,"0%"))}
if (all(paste(SUB_BD_INAF10$Etapa,SUB_BD_INAF10$Estado)!="3) ESPECIALISTA CSIG OBSERVADO")==T) {SUB_BD_INAF10=rbind(SUB_BD_INAF10,c("CSIG","Observado",0,"0%"))}
SUB_BD_INAF10$Registros=as.numeric(SUB_BD_INAF10$Registros)


#REORDENANDO
SUB_BD_INAF10=SUB_BD_INAF10[c(1:4,5,6),]
...
if (all(paste(SUB_BD_INAF11$Etapa,SUB_BD_INAF11$Estado)!="2) COORDINADOR OBSERVADO")==T) {SUB_BD_INAF11=rbind(SUB_BD_INAF11,c("Coordinador","Observado",0,"0%"))}
if (all(paste(SUB_BD_INAF11$Etapa,SUB_BD_INAF11$Estado)!="3) ESPECIALISTA CSIG OBSERVADO")==T) {SUB_BD_INAF11=rbind(SUB_BD_INAF11,c("CSIG","Observado",0,"0%"))}
...
#REORDENANDO
SUB_BD_INAF11=SUB_BD_INAF11[c(1,2,5,3,6,4),]
rownames(SUB_BD_INAF11) <- 1:nrow(SUB_BD_INAF11)
SUB_BD_INAF11=as.data.frame(SUB_BD_INAF11)
...
links=as.data.frame(matrix(c(
  ...
  1, 5, SUB_BD_INAF11[3,3], # LLENADOS APROBADOS 
  3, 6, SUB_BD_INAF11[4,3], # APROBADOS EN REVISION
  3, 7, SUB_BD_INAF11[5,3], # APROBADOS VALIDADOS
  3, 8, SUB_BD_INAF11[6,3]), # APROBADOS  OBSERVADOS
  byrow = T, ncol = 3))
