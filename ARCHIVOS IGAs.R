...
#CREAR TABLA RESUMEN
SUB_BD_INAF=summaryBy(N_ARCH ~ F_REG, FUN=sum, data =as.data.frame(SUB_BD_INAF),keep.names = T) #Para mantener el nombre de la variable usar: keep.names = T
...
animate(AVANCE_REG_DIA_TOT, duration = 12, fps = 15, width = 2200, height = 1600, res=250, renderer = gifski_renderer(loop = F))

anim_save("10) Avance ARCHIVOS acumulado total.gif")
...
################################################
#####  ACAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA  #####
################################################
...
animate(AVANCE_REG_DIA_TOT_VAL, duration = 12, fps = 15, width = 2200, height = 1600, res=250, renderer = gifski_renderer(loop = F))

anim_save("10.2) Avance archivo por proceso.gif")
...
animate(AVANCE_REG_DIA_TOT_Y_UFC, duration = 12, fps = 15, width = 2200, height = 1600, res=250, renderer = gifski_renderer(loop = F))

anim_save("11) Avance archivos acumulado Totales y UFC.gif")
...
#CREAR TABLA RESUMEN
SUB_BD_INAF1=summaryBy(N_ARCH ~ F_REG+SUB_SECT, FUN=fun1, data =as.data.frame(SUB_BD_INAF1),keep.names = T) #Para mantener el nombre de la variable usar: keep.names = T
...
animate(AVANCE_REG_DIA, duration = 12, fps = 15, width = 2200, height = 1600, res=250, renderer = gifski_renderer(loop = F))

anim_save("12) Evolución archivos por area.gif")
...
#CREAR TABLA RESUMEN
SUB_BD_INAF2=summaryBy(N_ARCH ~ F_REG+SUB_SECT, FUN=c(sum), data =as.data.frame(SUB_BD_INAF2),keep.names = T) #Para mantener el nombre de la variable usar: keep.names = T

#CREANDO CAMPO CONTEO ACUMULADO
SUB_BD_INAF2 = mutate(group_by(SUB_BD_INAF2,SUB_SECT),"AUX2" = cumsum(N_ARCH))


#GENERANDO GRÁFICO EN NIVELES
AVANCE_UF_DIA <- ggplot(SUB_BD_INAF2,aes(F_REG, N_ARCH, group = SUB_SECT, color = factor(SUB_SECT))) +
  geom_line(size=1.5) +
  scale_color_manual(values=c(OEFA_1,OEFA_3,OEFA_6,OEFA_5, OEFA_7, OEFA_4))+
  theme_minimal()+
  labs(x="",
       y="Cantidad de archivos",
       title = "Evolución de la cantidad de archivos de las Unidades Fiscalizables en INAF con al menos un IGA,\npor coordinación.",
       subtitle = paste("(Del ",
                        fecha_inicio,
                        " al ",
                        fecha_final,
...
animate(AVANCE_UF_DIA, duration = 12, fps = 15, width = 2200, height = 1600, res=250, renderer = gifski_renderer(loop = F))
...
SUB_BD_INAF3$AUX2=1
SUB_BD_INAF3=summaryBy(N_ARCH+AUX2 ~ F_REG+SUB_SECT, FUN=fun1, data =as.data.frame(SUB_BD_INAF3),keep.names = T) #Para mantener el nombre de la variable usar: keep.names = T
SUB_BD_INAF3$REGISTROS_PERCAP=SUB_BD_INAF3$N_ARCH/SUB_BD_INAF3$AUX2
...
animate(AVANCE_REG_PERCAP_DIA, duration = 12, fps = 15, width = 2800, height = 1600, res=250, renderer = gifski_renderer(loop = F))

# Save at gif:
anim_save("13) Llenado per cápita de ARCHIVOS por área.gif")
...
#CREAR TABLA RESUMEN
SUB_BD_INAF5=summaryBy(N_ARCH ~ F_REG+SUB_SECT+REGISTRADOR, FUN=fun1, data =as.data.frame(SUB_BD_INAF5),keep.names = T) #Para mantener el nombre de la variable usar: keep.names = T
SUB_BD_INAF5$AUX2=1
SUB_BD_INAF5=summaryBy(N_ARCH+AUX2 ~ F_REG+SUB_SECT, FUN=fun1, data =as.data.frame(SUB_BD_INAF5),keep.names = T) #Para mantener el nombre de la variable usar: keep.names = T
SUB_BD_INAF5$REGISTROS_PERCAP=SUB_BD_INAF5$N_ARCH/SUB_BD_INAF5$AUX2
...
ggsave("14) Tendencia llenado ARCHIVO per cápita por área.jpg",  width = 12, height = 7)
...
###############################################################
#####  Graficando avances 3: REGISTROS PER CAPITA (CELE)  #####
###############################################################
...
###############################################################
#####  Graficando avances 3: REGISTROS PER CAPITA (CHID)  #####
###############################################################
...
###############################################################
#####  Graficando avances 3: REGISTROS PER CAPITA (CIND)  #####
###############################################################
...
###############################################################
#####  Graficando avances 3: REGISTROS PER CAPITA (CPES)  #####
###############################################################
...
###############################################################
#####  Graficando avances 3: REGISTROS PER CAPITA (CRES)  #####
###############################################################
...
#REORDENANDO
SUB_BD_INAF11=SUB_BD_INAF11[c(1:2,6,3,5,4),]
rownames(SUB_BD_INAF11) <- 1:nrow(SUB_BD_INAF11)
